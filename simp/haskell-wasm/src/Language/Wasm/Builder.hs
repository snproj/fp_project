...
module Language.Wasm.Builder (
    ...
    if', loop, block, when, for, while, -- uncommented by Kenny Lu
    trap, unreachable,
    appendExpr, after, 
    genExpr, -- added by Kenny Lu
...
-- added by Kenny Lu
valueTypeToBlockType :: [ValueType] -> BlockType 
valueTypeToBlockType = Inline . Maybe.listToMaybe
...
if' res pred true false = do
    produce pred
    deep <- (+1) <$> ask
    appendExpr [If (valueTypeToBlockType $ asResultValue res) (genExpr deep $ true) (genExpr deep $ false)]
    return returnableValue

loop :: (Returnable res) => res -> GenFun res -> GenFun res
loop res body = do
    deep <- (+1) <$> ask
    appendExpr [Loop (valueTypeToBlockType $ asResultValue res) (genExpr deep $ body)]
    return returnableValue

block :: (Returnable res) => res -> GenFun res -> GenFun res
block res body = do
    deep <- (+1) <$> ask
    appendExpr [Block (valueTypeToBlockType $ asResultValue res) (genExpr deep $ body)]
    return returnableValue
