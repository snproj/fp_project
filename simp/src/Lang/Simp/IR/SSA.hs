  ...
  let labelsModdedVars :: [(Label, [String])]
      ...
      paWithPhis =
        map
          ...
          pa
   in -- Lab 3 Task 1.2 END
      foldl (\acc (l, phis, i) -> DM.insert l (l, phis, i) acc) DM.empty paWithPhis
...
convDTree k (Node l children) p g = case DM.lookup l p of
  ...
  Just li -> do
    ...
    pnj <-
      foldM
        ( \acc succ -> case DM.lookup succ acc of
...
convPhiLhs k phis = do
  ...
  return
    ( k2,
      sortOn
        ( \phi -> case phi of
            PhiAssignment dest operands stem -> stem
        )
        phis2
    )
  where
  ...
  let q :: [LabeledInstr]
      ...
      q1 =
        foldl
          ( \acc (l, phis, i) ->
              foldl
                ( \acc1 (PhiAssignment d oprs stem) ->
